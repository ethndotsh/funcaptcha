...
let parseToken = (token: string): TokenInfo =>
  Object.fromEntries(
    token.split("|").map((v) => v.split("=").map((v) => decodeURIComponent(v)))
  );
...
export class Session {
  ...
  constructor(token: string | GetTokenResult, sessionOptions?: SessionOptions) {
    ...
    if (!this.token.startsWith("token=")) this.token = "token=" + this.token;

    this.tokenInfo = parseToken(this.token);
    this.tokenInfo.mbio =
      typeof token !== "string" ? token.mbio ?? false : false;
  ...
  async getChallenge(): Promise<Challenge> {
    let res = await http(util.replacePath(this.tokenInfo.surl, "/fc/gfct/"), {
      method: "POST",
