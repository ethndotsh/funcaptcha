...
export abstract class Challenge {
  ...
  protected async getKey() {
    ...
    let response = await request(
      util.replacePath(this.data.tokenInfo.surl, "/fc/ekey."),
      {
        method: "POST",
        headers: {
          "User-Agent": this.userAgent,
  ...
  get variant() {
    return (
      this.data.game_data.game_variant || this.data.game_data.instruction_string
    );
  }

  get instruction() {
    return (
      this.data.string_table[
        `${this.data.game_data.gameType}.instructions-${this.variant}`
      ] ||
      this.data.string_table[
        `${this.data.game_data.gameType}.touch_done_info${
          this.data.game_data.game_variant
            ? `_${this.data.game_data.game_variant}`
            : ""
        }`
      ]
    );
...
export class Challenge1 extends Challenge {
  ...
  constructor(data: ChallengeData, challengeOptions: ChallengeOptions) {
    ...
    this.increment = parseInt(
      clr ? clr.replace("#", "").substring(3) : "28",
      16
    );
    this.increment =
      this.increment > 113 ? this.increment / 10 : this.increment;
  }
  ...
  async answer(answer: number): Promise<AnswerResponse> {
    ...
    else this.answerHistory.push(this.round(answer));
    ...
    let req = await request(
      util.replacePath(this.data.tokenInfo.surl, "/fc/ca"),
      {
        method: "POST",
        headers: {
          "User-Agent": this.userAgent,
...
export class Challenge3 extends Challenge {
  ...
  async answer(tile: number): Promise<AnswerResponse> {
    ...
    this.answerHistory.push(
      util.solveBreaker(
        !!this.data.game_data.customGUI.is_using_api_breaker_v2,
        this.data.game_data.customGUI.api_breaker,
        3,
        pos
      )
    );
    ...
    let requestedId = await crypt.encrypt(
      JSON.stringify({}),
      `REQUESTED${this.data.session_token}ID`
    );
    ...
    let req = await request(
      util.replacePath(this.data.tokenInfo.surl, "/fc/ca/"),
      {
        ...
        body: util.constructFormData({
          ...
          bio:
            this.data.tokenInfo.mbio &&
            "",
        }),
      }
    );
    let reqData = JSON.parse(req.body.toString());
